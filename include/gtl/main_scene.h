...
namespace scenes {
    ...
    class main_scene {
        ...
        std::atomic<bool> mutable draw_debug_{false};
        ...
        auto box_generator()
        {
            ...
            for (int i = 1; i < 10; ++i)
            {
                generators_.emplace_back(
                    dynamic_box{{vn::math::rand_neg_one_one() * 10.0f, vn::math::rand_zero_one() * 10.0f, vn::math::rand_zero_one() * 10.0f + 10.f},
                                {1.0f, 1.0f, 1.0f},
            ...
            for (unsigned i = 1; i < 100; ++i)
            {
                ...
                generators_.emplace_back(dynamic_jointed_boxes{
                    std::move(jointed_boxes_), gtl::entity::render_data{}.pack_entity_id(i + 200).pack_mesh_id(mesh_group_.index_of_mesh("cone")),
                    gtl::entity::id(i + 200)});
            }
            ...
            for (unsigned i = 1; i < 100; ++i)
            {
                ...
                for (unsigned j = 0; j < 4; ++j)
                {
                    jointed_boxes_.emplace_back(
                        dynamic_box{{head_x, head_y, head_z - (j * 1.0f)},
                                    {1.0f, 1.0f, 1.0f},
                                    Eigen::Quaternionf::Identity(),
                ...
                generators_.emplace_back(dynamic_jointed_boxes{
                    std::move(jointed_boxes_), gtl::entity::render_data{}.pack_entity_id(i + 700).pack_mesh_id(mesh_group_.index_of_mesh("rectangle")),
                    gtl::entity::id(i + 700)});
            }
        ...
        auto default_generator()
        {
            ...
            generators_.emplace_back(polymorphic_generator{
            ...
            // generators_.emplace_back(static_rectangle{{0.0f * si::meters, -100.0f * si::meters}, {200.0f * si::meters, 5.0f * si::meters}, 0.0f *
            // si::radians, {}});
            ...
            // {}});
            // generators_.emplace_back(static_rectangle{{-100.0f * si::meters, 0.0f * si::meters}, {5.0f * si::meters, 200.0f * si::meters}, 0.0f *
            // si::radians, {}});
            ...
            for (unsigned j = 0; j < 80; ++j)
            {
                ...
                for (unsigned i = 0; i < 4; ++i)
                {
                    jointed_boxes_.emplace_back(gtl::physics::commands::dynamic_rectangle{{x, y - ((i * 1.0f) * si::meter)},
                                                                                          ...
                                                                                          angle,
                                                                                          gtl::entity::render_data{}.pack_entity_id(j + 600).pack_mesh_id(0)});
                }
            ...
            for (unsigned j = 80; j < 160; ++j)
            {
                ...
                // for (unsigned i = 0; i < 4; ++i) {
                //  jointed_boxes_.emplace_back(
                generators_.emplace_back(gtl::physics::commands::dynamic_rectangle{
                    {x, y}, {1.0f * si::meter, 1.0f * si::meter}, angle, gtl::entity::render_data{}.pack_entity_id(j + 600).pack_mesh_id(1)});
                //}
        ...
        auto test_generator()
        {
            ...
            for (unsigned j = 0; j < 50; ++j)
            {
                ...
                for (unsigned i = 0; i < 4; ++i)
                {
                    jointed_boxes_.emplace_back(dynamic_rectangle{{x, y - ((i * 1.0f) * si::meter)},
                                                                  {1.0f * si::meter, 1.0f * si::meter},
                                                                  angle,
                                                                  gtl::entity::render_data{}.pack_entity_id(j + 600).pack_mesh_id(0)});
                }
            ...
            for (unsigned j = 50; j < 100; ++j)
            {
                ...
                //  jointed_boxes_.emplace_back(
                generators_.emplace_back(
                    dynamic_rectangle{{x, y}, {1.0f * si::meter, 1.0f * si::meter}, angle, gtl::entity::render_data{}.pack_entity_id(j + 600).pack_mesh_id(1)});
                //}
                // generators_.emplace_back(dynamic_jointed_rectangles{std::move(jointed_boxes_)});
            }
    ...
    public:
        ...
        void toggle_debug()
        {
            draw_debug_.store(!draw_debug_.load(std::memory_order_acquire), std::memory_order_release);
        }
        ...
        {
            func([&](auto&&... ps) {
                Eigen::Matrix4f view_ = Eigen::Affine3f{Eigen::Translation3f{0.0f, -(camera_height_ / boost::units::si::meter), 60.0f}
                ...
                if (draw_debug_.load(std::memory_order_acquire))
        ...
        gtl::event handle_events(ResourceManager& resource_callback_, YieldType& yield) const
        {
            ...
            auto top_level_handler
                = vn::make_lambda_visitor([&](ev::mouse_event const& m) { mouse_handler(m); },
                                          ...
                                          [&](ev::keydown const& k) {
                                              switch (k.key)
                                              {
                                                  ...
                                                  case k::A:
                                                  {
                                                      ...
                                                      physics_task_queue_.insert(dynamic_rectangle{{0.0f * si::meter, 0.0f * si::meter},
                                                                                                   {0.5f * si::meter, 0.5f * si::meter},
                                                                                                   0.0f * si::radians,
                                                                                                   gtl::entity::render_data{}.pack_entity_id(888)});
                                                  }
                                                  break;
                                                  case k::K:
                                                  {
                                          ...
                                          [&](ev::dpad_pressed const& e) {
                                              ...
                                              physics_task_queue_.insert(boost_object_vec{selected_id, e.x, e.y}); // HACK
                                              // std::cout << "dpad..\n"; //physics_task_queue_.swap_in(task_local_);
                                          },
