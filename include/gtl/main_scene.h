...
namespace scenes {
    ...
    class main_scene {
        //: public
        // gtl::event_listener<main_scene,gtl::command_variant> {

        gtl::swap_vector<gtl::physics::generator> mutable physics_task_queue_; // TODO consider
        // using
        // vn::single_consumer_queue
    ...
    public:
        main_scene(gtl::d3d::device& dev, gtl::d3d::swap_chain& swchain,
            gtl::d3d::command_queue& cqueue)
            : physics_task_queue_{ []() {
                ...
                generators_.emplace_back(
                    static_box{ { 0.0f * si::meters, -100.0f * si::meters },
                ...
                generators_.emplace_back(
                    static_box{ { 0.0f * si::meters, 100.0f * si::meters },
                        ...
                        {} });
                generators_.emplace_back(
                    static_box{ { -100.0f * si::meters, 0.0f * si::meters },
                ...
                for (unsigned j = 0; j < 80; ++j) {
                    ...
                    generators_.emplace_back(
                        dynamic_jointed_boxes{ std::move(jointed_boxes_) });
                }
                ...
                for (unsigned j = 80; j < 160; ++j) {
                    ...
                    generators_.emplace_back(dynamic_box{
                        { x, y },
                        { 1.0f * si::meter, 1.0f * si::meter },
                        angle,
            ...
            , physics_camera_{ { 0.0f * boost::units::si::meters,
                                   0.0f * boost::units::si::meters },
                { 1.0f * boost::units::si::meters,
                    1.0f * boost::units::si::meters },
                gtl::physics::angle<float>{
                    45.0f * boost::units::degree::degree },
        ...
        {
            func([&](auto&&... ps) {
                Eigen::Matrix4f cam_transform_ = Eigen::Affine3f{
                    Eigen::Scaling(1.0f / (camera_height_ / boost::units::si::meter)) //}.matrix();
                    * Eigen::AngleAxisf{ 0.2f, Eigen::Vector3f{ 0.0f, 0.0f, -1.0f } } * Eigen::AngleAxisf{ 0.2f, Eigen::Vector3f{ -1.0f, 0.0f, 0.0f } }
                }.matrix();

                draw_task_queue_.consume(
                    [](auto&& f) { f(); }); // execute our waiting tasks..
                swirl_effect_.draw(ps..., current_id_,
        ...
        gtl::event handle_events(ResourceManager& resource_callback_,
            YieldType& yield) const
        {
            ...
            while (!same_type(yield().get(), ev::exit_immediately{})) {
                if (same_type(yield.get(), ev::keydown{})) {
                    ...
                    case k::Q: {
                        std::cout
                            << "swirl_effect(): q pressed, exiting A from route 0 (none == "
                            << count << ")\n";
                        return gtl::events::exit_state{ 0 };
                    } break;
                    case k::A: {
                        std::cout
                            << "swirl_effect(): A pressed, generating new object.. \n";
                        task_local_.emplace_back(
                            dynamic_box{ { 0.0f * si::meter, 0.0f * si::meter },
                    ...
                    case k::K: {
                        std::cout << "swirl_effect(): k pressed, throwing (none == "
                                  << count << ")\n";
                    ...
                    case k::R: {
                        ...
                        if (same_type(resizing_event_context(yield), ev::keep{})) {
                            ...
                            resource_callback_(
                                gtl::commands::resize{ dims.first, dims.second }, []() {});
                        }
                    ...
                    std::cout << "camera's new height == "
                              << camera_height_ / boost::units::si::meter << "\n";
                    ...
                    auto& coord_ = boost::get<ev::mouse_lbutton_down>(yield.get().value())
                                       .coord; // HACK fix this
                    int mx = GET_X_LPARAM(coord_);
                    int my = GET_Y_LPARAM(coord_);

                    draw_task_queue_.insert(
                        [=, this]() { this->imgui_adapter_.mouse_down(mx, my); });
                    resource_callback_(gtl::commands::get_audio_adapter{},
                        [](auto& aud) { aud.play_effect("click"); });
                    // task_local_.emplace_back(destroy_object_implode{id});
                    // physics_task_queue_.swap_in(task_local_);
                } else if (same_type(yield.get(), ev::mouse_lbutton_up{})) {
                    auto& coord_ = boost::get<ev::mouse_lbutton_up>(yield.get().value())
                                       .coord; // HACK fix this
                    int mx = GET_X_LPARAM(coord_);
                    int my = GET_Y_LPARAM(coord_);
                    draw_task_queue_.insert(
                        [=, this]() { this->imgui_adapter_.mouse_up(mx, my); });
                    ...
                    // << "\n";
                    // uint16_t id = current_id_.load(std::memory_order_relaxed);
                    task_local_.emplace_back(boost_object_vec{
                        selected_id, dpad_press.x / 400.0f, dpad_press.y / 400.0f });
                    physics_task_queue_.swap_in(task_local_);
                } else if (same_type(yield.get(), ev::none{})) {
                    count++;
                } else if (same_type(yield.get(), ev::mouse_moved{})) {
                    swirl_effect_.set_mouse_coords(
                        boost::get<ev::mouse_moved>(yield.get().value()).coord);

                    auto& coord_ = boost::get<ev::mouse_moved>(yield.get().value())
                                       .coord; // HACK fix this
                    int mx = GET_X_LPARAM(coord_);
                    int my = GET_Y_LPARAM(coord_);
                    draw_task_queue_.insert(
                        [=, this]() { this->imgui_adapter_.mouse_move(mx, my); });
