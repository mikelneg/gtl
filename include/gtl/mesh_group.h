...
/*-----------------------------------------------------------------------------
    ...
    template <typename Vertex, typename Index, typename Key>
    class mesh_group
-----------------------------------------------------------------------------*/
...
namespace gtl {
    ...
    struct mesh_group {     // HACK hackish..
        ...
        void add_mesh(K key, V v, I i, size_t bone_count = 0) {
            ...
            if (it != end(offset_data)) return; // already contains key..            
        ...
        inline void apply(K key, F func) const {
            auto it = find_if(begin(offset_data),end(offset_data),[&](auto const& entry){ return entry.key == key; });
            if (it == end(offset_data)) throw std::out_of_range{__func__}; // key not found..       
            auto const& e = *it;        
        ...
        auto vertex_data() { return vertex_buffer.data(); }
        auto index_data() { return index_buffer.data(); }

        size_t vertex_count() const { return vertex_buffer.size(); }
        size_t index_count() const { return index_buffer.size(); }
        size_t mesh_count() const { return offset_data.size(); }
    };
