...
class mesh_group {
    ...
    struct entry {
        unsigned vertex_offset, index_offset, vertex_count, index_count, weights_per_vertex;
    };

    std::vector<V> vertex_buffer;
    std::vector<I> index_buffer;
    std::vector<std::pair<K, entry>>
        offset_data; // we use vector<> here rather than something like flat_map<> for offset_data because we need to preserve insertion order
...
public:
    ...
    void add_mesh(K key, VBuffer const& v, IBuffer const& i, unsigned weights_per_vertex)
    {
        ...
        if (it != end(offset_data))
            throw std::runtime_error{"mesh_group::add_mesh() key already inserted..\n"};

        offset_data.emplace_back(std::move(key), entry{static_cast<unsigned>(vertex_buffer.size()), static_cast<unsigned>(index_buffer.size()),
                                                       static_cast<unsigned>(v.size()), static_cast<unsigned>(i.size()), weights_per_vertex});
        using std::begin;
        using std::end;
    ...
    void add_armature(K key, mesh::armature armature_)
    {
        ...
        if (it != end(armature_data))
            throw std::runtime_error{"mesh_group::add_armature() key already inserted..\n"};
    ...
    void apply(K const& key, F func) const
    {
        auto it = find_if(begin(offset_data), end(offset_data), [&](auto const& e) { return e.first == key; });
        if (it == end(offset_data))
            throw std::runtime_error{"mesh_group::apply() key not found..\n"};
    ...
    unsigned index_of_mesh(K const& key) const
    {
        auto it = find_if(begin(offset_data), end(offset_data), [&](auto const& e) { return e.first == key; });
        if (it == end(offset_data))
            throw std::runtime_error{"mesh_group key not found..\n"};
    ...
    auto armature(K const& key) const
    {
        auto it = find_if(begin(armature_data), end(armature_data), [&](auto const& e) { return e.first == key; });
        if (it == end(armature_data))
            throw std::runtime_error{"mesh_group::armature() key not found..\n"};
        return it->second;
    }
