...
namespace gtl {
    ...
    public:        
        ...
        void reset(T* t = nullptr) noexcept {  // Cleanup if ptr_ is non-null
            if (ptr_) { Cleanup::operator()(ptr_); ptr_ = t; }                        
        }

        T*& expose_ptr() noexcept { this->reset(); return ptr_; } // Cleanup; resets internal pointer and returns it by reference        
        ...
        friend T*& expose(intrusive_ptr& t) noexcept { // Cleanup; resets internal pointer and returns it by reference
            return t.expose_ptr(); 
        }        
    };               
