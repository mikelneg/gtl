...
public:
    ...
    struct scene_hash {
        std::size_t operator()(scene_type const& s) const
        {
            return boost::apply_visitor(scenes::detail::variant_hash<scene_type>{}, s);
        }
    };

    struct scene_eq {
        bool operator()(scene_type const& lhs, scene_type const& rhs) const
        {
            return boost::apply_visitor(vn::visitors::weak_equality{}, lhs, rhs);
        }
    };
