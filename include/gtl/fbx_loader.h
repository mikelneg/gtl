...
namespace fbx {
    ...
    struct fbx_bone {                    
        ...
        static std::string root_id() { return "root"; } 
        ...
        fbx_bone(FbxMatrix const& global_transform, FbxMatrix const& local_transform,
             float length, id_type parent) 
            : g_transform_{global_transform}, 
              l_transform_{local_transform}, 
              length_{length}, 
              parent_{std::move(parent)} {}
    
        void add_child(std::string child) { children_.emplace_back(std::move(child)); }
    
        std::pair<FbxVector4,FbxVector4> head_and_tail() const { 
            return std::make_pair(g_transform_.MultNormalize({0.0,0.0,0.0,1.0}),
                                  g_transform_.MultNormalize({length_,0.0,0.0,1.0})); 
        }                                           

        friend std::ostream& operator<<(std::ostream& str, fbx_bone const& b) { return str << b.parent_ << ","; }
    };
    ...
    public:
        ...
        auto vertex_positions() const { return positions_; }
        auto vertex_normals() const { return normals_; }
