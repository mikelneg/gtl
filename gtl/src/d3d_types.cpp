...
namespace _12_0 {
    ...
    swap_chain::swap_chain(gtl::window& win, command_queue& cqueue) 
    {
        ...
        tmp_ptr.release();  // we must release() here since we do not want the resource destroyed when we leave        
    ...
    {        	    
		...
		throw_on_fail(dev->CreateGraphicsPipelineState(&desc,__uuidof(type),reinterpret_cast<void**>(&expose_ptr()))
                      ,__func__);
    }
    ...
    void fence::wait_for_previous_frame(swap_chain& swchain_, command_queue& cqueue_)
    {        
	    auto const local_value = fence_value.load(std::memory_order_relaxed);
        throw_on_fail(cqueue_->Signal(this->get(),local_value)
                      ,__func__);
	    fence_value.fetch_add(1,std::memory_order_relaxed);
