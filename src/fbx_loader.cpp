...
namespace mesh {
    namespace fbx {
        ...
        namespace {
            inline Eigen::Matrix4f convert_matrix(fbxsdk::FbxMatrix const& in)
            {
                Eigen::Matrix4f out;
                std::transform(static_cast<double const*>(in), static_cast<double const*>(in) + 16, out.data(),
                               [](auto const& v) { return static_cast<float>(v); });
                return out;
            }
            Eigen::Vector4f convert_vector(FbxVector4 const& v, float w)
            {
                return {static_cast<float>(v[0]), static_cast<float>(v[1]), static_cast<float>(v[2]), w};
            }
            Eigen::Vector4f convert_color(FbxColor const& c)
            {
                return {static_cast<float>(c[0]), static_cast<float>(c[1]), static_cast<float>(c[2]), static_cast<float>(c[3])};
            }

            inline Eigen::Matrix4f convert_matrix_handedness(fbxsdk::FbxMatrix const& in)
            {
                //  { rx, ry, rz, 0 }
                //  { ux, uy, uz, 0 }
                //  { lx, ly, lz, 0 }
            ...
            inline Eigen::Vector4f convert_vector_handedness(fbxsdk::FbxVector4 const& in, float w)
            {
            ...
            std::string name(FbxNodeAttribute::EType e)
            {
                switch (e)
                {
                    ...
                    case FbxNodeAttribute::EType::eLine:
                        return "eLine";
                    default:
                        return "Unknown_FbxNodeAttribute";
                }
            }
                ...
                for (int i = 0; i < mesh->GetPolygonCount(); ++i)
                {
                    for (int j = 0, sz = mesh->GetPolygonSize(i); j < sz; ++j)
                    {
                        ...
                        normals_.emplace_back(
                            convert_vector_handedness(tmp_normal, 0.0f)); // w == 0.0f; also, if normals are not mapped tmp_normal remains unchanged
                        indices_.emplace_back(j + (i * sz));              // control_point_index );   // ignoring actual index for now..

                        if (vertex_bone_data_)
                        {
                            bones_.emplace_back((*vertex_bone_data_)[control_point_index].indices_, (*vertex_bone_data_)[control_point_index].weights_);
                        }
                    }
                }
            };
        ...
        void fbx_loader::load_armature(FbxNode* p, fbxsdk::FbxAMatrix const& mesh_transform)
        {
            ...
            auto add_entry = [&](auto* node, auto const& parent_id) {
                armature_.try_emplace(node->GetName(), node->EvaluateGlobalTransform(), node->EvaluateLocalTransform(), node->GetSkeleton()->LimbLength.Get(),
                                      parent_id);
            };
        ...
        gtl::mesh::armature fbx_loader::convert_armature() const
        {

            if (!bone_names_)
                throw std::runtime_error{"fbx_loader::armature() requires bone_names_"};
            auto const& names_ = *bone_names_;

            auto convert_bone = [&](fbx_bone const& b) -> gtl::mesh::bone {
                auto parent_id_ = names_.count(b.parent_) > 0 ? names_.at(b.parent_) : gtl::mesh::bone::root_id();
                gtl::mesh::bone return_bone{convert_matrix(b.g_transform_), convert_vector_handedness(b.head_and_tail().second, 1.0f), parent_id_};
