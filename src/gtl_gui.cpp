...
namespace gui {  
    ...
    void textbox::set_focus(gui_listener_pull& p) {        
        ...
        while (!has_variant_type<gtl::events::exit_all>(msg(p.get()).value())) {
            if (has_variant_type<gtl::events::none>(msg(p.get()).value())) {
    ...
    void textbox::listen(gui_listener_pull& p) {
        ...
        while (!has_variant_type<gtl::events::exit_all>(msg(p.get()).value())) {
            ...
            if (has_variant_type<gtl::events::mouse_click>(msg(p.get()).value())) {        
                try {
                    std::cout << "[textbox noticed the <click> message for " << 
                        (boost::get<gtl::events::mouse_click>(msg(p.get()).value()).id) << "!]\n";
                }
                catch(...) {
