...
namespace mesh {
    ...
    namespace {
        ...
        inline Eigen::Matrix4f convert_matrix_handedness(fbxsdk::FbxAMatrix const& in)
        {
            //  { rx, ry, rz, 0 }
            //  { ux, uy, uz, 0 }
            //  { lx, ly, lz, 0 }
    ...
    struct mesh_loader::priv_impl {
        ...
        decltype(auto) loader()
        {
            return loader_;
        }

        priv_impl(std::string filename) : loader_(std::move(filename))
        {
        }
    };

    mesh_loader::mesh_loader(std::string filename, tags::mesh_format_fbx) : impl_{std::make_unique<priv_impl>(std::move(filename))}
    {
    }
...
*/
    ...
    std::vector<renderer_vertex_type> mesh_loader::assembled_vertices() const
    {
        ...
        assert(positions.size() == normals.size() && positions.size() == uvs.size() && positions.size() == bones.size());
    ...
    boost::container::flat_map<bone::id_type, bone> mesh_loader::armature() const
    {
        return impl_->loader_.convert_armature();
    }

    boost::container::flat_map<std::string, mesh::bone::id_type> mesh_loader::armature_ids() const
    {
